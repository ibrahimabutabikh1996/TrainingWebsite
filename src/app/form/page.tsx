...
function FormContent() {
  ...
  /* The payment gate stands in front of step 1 for a new registration: the fee
     is settled over WhatsApp and the transfer slip attached before any question
     is asked. A renewal is already a paying member, so it opens on step 1. */
  const [paymentDone, setPaymentDone] = useState(() => isRenew && Boolean(profileId));
  ...
  const stepProps = { formData, update, planLocked: Boolean(searchParams.get("plan")) };
      ...
      <div className="form-shell form-shell--centered">
        <article className="form-sheet done-sheet">
          ...
          <p className="done-summary">
            ...
            <br />
            {t("form_done_plan")}: <strong>{planLabel(formData.plan)}</strong>
          </p>
          <p className="done-text">{t("form_done_text")}</p>
