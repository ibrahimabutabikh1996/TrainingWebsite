...
export default function WeightLog({ profile }: WeightLogProps) {
  ...
  sortedLogs.forEach(log => {
    chartData.push({
      date: log.date,
      displayDate: new Date(log.date).toLocaleDateString("ar-EG", { month: 'short', day: 'numeric' }),
      weight: log.weight,
      isOriginal: false
    });
  });
  ...
  return (
    <div className="home-overview-container" style={{ maxWidth: 1000 }}>
      <div className="home-hero-banner" style={{ marginBottom: "var(--space-6)" }}>
        <div className="home-hero-text">
          <h2>سجل الوزن الأسبوعي</h2>
          <p>تابع تطور وزنك بدقة واحترافية. قم بتسجيل وزنك كل يوم جمعة صباحاً للحصول على أفضل النتائج ولمساعدة الكابتن في متابعة تقدمك.</p>
        </div>
        <Icon name="monitor_weight" style={{ fontSize: "56px", color: "var(--primary-on-tint)", flexShrink: 0 }} />
      </div>
            ...
            <div className="home-stat-card" style={{ flex: "1 1 0", padding: 0, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "400px" }}>
              ...
              <div style={{ overflowY: "auto", flex: 1 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "right" }}>
                  ...
                  <tbody>
                    {chartData.slice().reverse().map((log, index, arr) => {
                        ...
                        <tr key={log.date} style={{ 
                          ...
                          <td style={{ padding: "var(--space-4) var(--space-6)", fontSize: "0.95rem" }}>
                            {log.isOriginal ? (
                              ...
                              <span style={{ color: "var(--text)" }}>{new Date(log.date).toLocaleDateString("ar-EG", { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                            )}
                          </td>
