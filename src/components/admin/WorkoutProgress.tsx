...
export default function WorkoutProgress({
  ...
  const dayExercises = new Map<string, { name: string; sets: Map<number, Row> }>();
    ...
    <details className="crm-modal-section" style={{ background: 'var(--bg2)', padding: '24px', borderRadius: "var(--radius-xl)", border: '1px solid var(--border-strong)' }}>
      ...
      <div style={{ flex: '0 0 400px', maxWidth: '100%', padding: 'var(--space-4)', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius-lg)' }}>
        <TrainingCalendarMonth
          ...
          onSelectDay={(d) => setSelectedDate((cur) => (cur === d ? null : d))}
        />
      </div>
      )}
                    ...
                    >
                      {cell ? (
                        <>
                          ...
                          <div style={{ color: "var(--text)", fontWeight: 600, fontSize: "0.98rem", lineHeight: 1.3 }}>
                            {cell.weight != null ? cell.weight : "—"}
                            {cell.weight != null && (
                              <span style={{ color: "var(--text-muted)", fontWeight: 500, fontSize: "0.68rem", marginInlineStart: 3 }}>
                                كغم
                              </span>
                            )}
                          </div>
